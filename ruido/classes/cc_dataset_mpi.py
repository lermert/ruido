...
class CCData(object):
    ...
    def filter_data(self, taper_perc=0.1, filter_type="bandpass",
        ...
        if rank == 0 and nrest > 0:
            ...
            self.data[ndata - nrest: ndata] = np.array(filt_rest)
    ...
    def measure_dvv_par(self, ref, f0, f1, method="stretching",
...
run measure_dvv_ser on one process.")
        if rank == 0:
            ...
            to_measure = self.data[0: ndata - nrest]
            timestamps = self.timestamps[0: ndata - nrest]
            npts = self.npts
            fs = self.fs
            lag = self.lag
        else:
...
class CCDataset(object):
    ...
    def __init__(self, inputfile, ref=None):
        """
        :type inputfile: string
